...
public class BookServiceImpl implements BookService {
    ...
    public BookDTO getBookByName(String name) {
        return bookRepository.findByName(name).map(book -> mapper.map(book, BookDTO.class))
            .orElseThrow(() -> {
                String message = messageSource.getMessage(MessageKeys.ERROR_BOOK_NOT_FOUND, new Object[]{name}, LocaleContextHolder.getLocale());
    ...
    public BookDTO addBook(BookDTO dto) {
        ...
        if (bookRepository.existsByName(dto.getName())) {
            String message = messageSource.getMessage(MessageKeys.ERROR_BOOK_ALREADY_EXISTS,
